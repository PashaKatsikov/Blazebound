...
//        • lib/midway/cipher/unmask.dart          (family + salt + streamLen)
//        • lib/midway/brief/masked_bytes.dart      (encoded byte arrays)
//        • lib/midway/brief/midway_brief.dart      (identity + timings)
//        • lib/midway/brief/policy_links.dart      (public URLs)
//        • lib/midway/rigging/keybox.dart          (storage key prefix)
//        • lib/midway/rigging/push_channel.dart    (channel id + name)
//        • lib/midway/booth/portal_booth.dart      (upload channel name)
...
void _writeVeilCodec(Mutation m, {required bool dryRun}) {
  ...
  const String path = 'lib/midway/cipher/unmask.dart';
...
void _writeVeiledBytes(Mutation m, {required bool dryRun}) {
  ...
  const String path = 'lib/midway/brief/masked_bytes.dart';
...
import '../cipher/unmask.dart';

// ============================================================
// MASKED BYTES — generated by tool/forge/mint.dart
// ============================================================
...
void _writeRelayConfig(Mutation m, {required bool dryRun}) {
  ...
  const String path = 'lib/midway/brief/midway_brief.dart';
...
void _writeLegalUrls(Mutation m, {required bool dryRun}) {
  ...
  const String path = 'lib/midway/brief/policy_links.dart';
  final File file = File(path);
  if (!file.existsSync()) _bail('missing $path');
  String src = file.readAsStringSync();
  src = _replaceTopLevelString(
      src, 'siteUrl', m.endpoints['homeUrl'] as String? ?? '');
  src = _replaceTopLevelString(
      src, 'policyUrl', m.endpoints['privacyUrl'] as String? ?? '');
  src = _replaceTopLevelString(
      src, 'helpUrl', m.endpoints['supportUrl'] as String? ?? '');
...
void _writeBeaconKeystore(Mutation m, {required bool dryRun}) {
  ...
  const String path = 'lib/midway/rigging/keybox.dart';
  final File file = File(path);
  if (!file.existsSync()) _bail('missing $path');
  String src = file.readAsStringSync();
...
void _writeAlertChannel(Mutation m, {required bool dryRun}) {
  const String path = 'lib/midway/rigging/push_channel.dart';
  final File file = File(path);
  if (!file.existsSync()) _bail('missing $path');
  String src = file.readAsStringSync();
  ...
  src = src.replaceFirstMapped(
    RegExp(r"const String kPushChannelId = '[^']*';"),
    (Match _) => "const String kPushChannelId = '$id';",
  );
  src = src.replaceFirstMapped(
    RegExp(r"const String kPushChannelName = '[^']*';"),
    (Match _) => "const String kPushChannelName = '$label';",
  );
  _writeFile(path, src, dryRun: dryRun);
}

void _writePortalStage(Mutation m, {required bool dryRun}) {
  const String path = 'lib/midway/booth/portal_booth.dart';
  final File file = File(path);
  if (!file.existsSync()) _bail('missing $path');
  String src = file.readAsStringSync();
